...
pexGAUS <- function (q, mu = 5, sigma = 1, tau = 1, lower_tail = TRUE, log_p = FALSE)
{
    if (sigma <= 0) return(rep(NA,length(q)))
    if (tau <= 0) return(rep(NA,length(q)))

    if (sigma < 1e-4)
      return(pexp(q-mu,1/tau,log=log_p,lower.tail=lower_tail)) # shifited exponential
    ...
    cdf <- ifelse(is.finite(q),
      ifelse(tau > 0.05 * sigma,
        pnorm((q - mu)/sigma) - exp(log(pnorm(z/sigma)) +
        ((mu + (sigma^2/tau))^2 - (mu^2) - 2 * q * ((sigma^2)/tau))/(2 * sigma^2)),
        pnorm(q, mean = mu, sd = sigma)),ifelse(q<0,0,1)
    )
    if (lower_tail == TRUE)
      cdf <- cdf
    else cdf <- 1 - cdf
...
dexGAUS <- function (x, mu = 5, sigma = 1, tau = 1, log_d = FALSE)
{
    if (sigma <= 0) return(rep(NA,length(x)))
    if (tau <= 0) return(rep(NA,length(x)))

    if (sigma < 1e-4)
      return(dexp(x-mu,1/tau,log=log_d)) # shifited exponential
...
rEXG <- function (n,mu=.5,sigma=.05,tau=.1,a=-Inf,b=Inf)
{
    ...
    if (any(!ok)) {
      nneed <- sum(!ok)
      if (any(ok)) nget <- ceiling(1.1*nneed/mean(ok)) else nget <- nneed
      rtok <- numeric(nneed)
      full <- logical(nneed)
      repeat {
        ...
        if (any(ok)) nget <- ceiling(nneed*2/mean(ok)) else nget <- nneed
      }
      out[!ok] <- rtok
    }
    out
...
dEXG <- function (x, mu = 5, sigma = 1, tau = 1, a=-Inf, b = Inf, log_d = FALSE)

{
  ...
  out <- dexGAUS(x[ok],mu,sigma,tau,log_d=FALSE)/(Fb-Fa)
  if (log_d) log(out) else out
}
...

{
  ...
  if (a == -Inf) Fa <- 0 else Fa <- pexGAUS(a,mu=mu,sigma=sigma,tau=tau)
  if (b == Inf) Fb <- 1 else Fb <- pexGAUS(b,mu=mu,sigma=sigma,tau=tau)
  out[ok] <- (pexGAUS(q[ok],mu,sigma,tau,log_p=FALSE,lower_tail=lower_tail)-Fa)/(Fb-Fa)
  if (log_p) log(out) else out
}
  ...
  # density for single accumulator
{
  ...
  t <- t-t0
  ok <- t > 0
  out[ok] <- digt(t[ok],k=B+A/2,l=v,a=A/2)
  out
}
    ...
    # cumulative density for single accumulator
{
    ...
    t <- t-t0
    ok <- t > 0
    out[ok] <- pigt(t[ok],k=B+A/2,l=v,a=A/2)
    out
}
  ...
  # Generates defective PDF for responses on node=1, dt (decison time) is a vector of times
{
  ...
  # Winner
  dt[1,is.go] <- (1-gf[1])*dWald(dt[1,is.go],A=A[1],v=v[1],B=B[1],t0=t0)
  if (n_acc > 1) for (i in 2:n_acc)
    dt[1,is.go] <- dt[1,is.go]*(1-pWald(dt[i,is.go],A=A[i],v=v[i],B=B[i],t0=t0))
...
n1Waldexg <- function(dt,v,B,A,t0=0,minEXG=0,Si=1)
  # Generates defective PDF for responses on node=1, dt (decision time) is a vector of times
{
  B[B<0] <- 0 # Protection for negatives
  A[A<0] <- 0
  n_acc <- ifelse(is.null(dim(v)),length(v),dim(v)[1])
  ...
  # Winner
  if (Si==1)
    dt[1,  ] <- dEXG(dt[1,  ],mu = v[1],sigma = B[1],tau = A[1],a=minEXG) else
    dt[1,  ] <- dWald(dt[1,  ],A=A[1],v=v[1],B=B[1],t0=t0)
  if (n_acc > 1) for (i in 2:n_acc)
    if (Si==i)
      dt[1,  ] <- dt[1,  ]*(1-pEXG(q=dt[i,  ],mu=v[i],sigma=B[i],tau=A[i],a=minEXG)) else
      dt[1,  ] <- dt[1,  ]*(1-pWald(dt[i,  ],A=A[i],v=v[i],B=B[i],t0=t0))

  dt[1,]
}


stopfn_rdex <- function(t,vj,Bj,Aj,t0,SSD,minEXG,Si)
    {
      t0S <- rep(0,length(vj))
      t0S[-Si] <- t0S[-Si]+SSD
      t0S[Si] <- t0S[Si]+t0
      dt <- matrix(rep(t,each=length(vj)),nrow=length(vj))+t0S
      n1Waldexg(dt[,,drop=FALSE],v=vj,B=Bj,A=Aj,t0=0,minEXG=minEXG,Si=Si)
}
...
rWaldss <- function (n, v, B, t0, A=0, tf=0, gf=0, ts = 0, minEXG=0,
  ...
  # ts = slope of slowing (speeding if negative) over TRIALS, B - ts*TRIALS
...
n1PDF.Waldss <- function(rt,v,B,t0,A=0,tf=0,gf=0,ts=0,minEXG=0,
...

  {
    ...
    if ( any(!is.stop) )
    {
      ...
      {
        rt[1,!is.stop] <- (1-gf)*(
          tf*n1Wald(rt[-Si,!is.stop,drop=FALSE],v=v[-Si],B=B[-Si],A=A[-Si],t0=0) +
            (1-tf)*n1Waldexg(rt[,!is.stop,drop=FALSE],v=v,B=B,A=A,t0=0,
                             minEXG=minEXG[1],Si=Si)
        )
      } else
   ...
   if ( any(is.stop) ) for (j in pj) {
      if ( !is.finite(SSD[j]) ) tmp <- 0 else {
        ...
        tmp <- my.integrate(f=stopfn_rdex,lower=minEXG[1]-t0,
                            vj=v,Bj=B,Aj=A,t0=t0,SSD=SSD[j],
                            minEXG=minEXG[1],Si=Si)
      }
...
# #### ABCD not updated ----
# rWaldssABCD <- function (n, v, B, t0,v0,vT,vF,aT,aF,minEXG=-Inf,
...
#       if ( any(is.na(staircase)) ) {           # STOP fixed SSD
#         # Set v
#         v[iT,isSS] <- v0 + (vT-v0)*(1-exp(-aT*SSD[isSS]))
#         v[iF,isSS] <- v0 + (vF-v0)*(1-exp(-aF*SSD[isSS]))
#
#         # add SSD to stop accumulator
#         t0S[1,] <- t0S[1,] + SSD
#         out <- rWaldexgRace(n,v=v,B=B,A=A,t0=t0S,a=minEXG)
...
#               v[iT,i] <- v0 + (vT-v0)*(1-exp(-aT*SSDi))
#               v[iF,i] <- v0 + (vF-v0)*(1-exp(-aF*SSDi))
...
#
#
# n1PDF.WaldssABCD <- function(rt,v,B,t0,v0,vT,vF,aT,aF,
...
#       p <- round(SSD[is.stop],6)
#       pj <- c(1:sum(is.stop))[!duplicated(p)] # index of unique SSD
#     } else {
#       B[-Si,] <- B[-Si,] + ts*TRIALS
#       p <- apply(
#         rbind(B[,is.stop,drop=FALSE],round(SSD[is.stop],6)),
#         2,paste,collapse="")
...
#
#     if ( any(is.stop) ) for (j in pj) {
#       if ( !is.finite(SSD[is.stop][j]) ) tmp <- 0 else {
#         # if (minEXG==-Inf) lower <- -1 else lower <- pmin(minEXG[1]-t0,0)
#         tmp <- my.integrate(f=stopfn,lower=minEXG[1]-t0,vj=v[,j],minEXG=minEXG[1],
#                             Bj=B[,j],Aj=A[,j],t0=t0,SSD=SSD[is.stop][j],Si=Si)
#       }
#       rt[1,is.stop][p %in% p[j]] <- gf +(1-gf)*(1-tf)*tmp
#     }
#     rt[1,]
