...
n1PDFfixedt0_tr_diff.norm=function(dt,A,b,mean_v,sd_v,t0, 
                           posdrift=TRUE,robust = FALSE) 
# Allows t0 to differ among accumulator
# Generates defective PDF for responses on node= 1
# dt (decison time) is a matrix with length(mean_v) rows, one row for
# each accumulator to allow for different start times
{
  ...
  n_acc <- length(mean_v)
  A <- matrix(rep(A,each=length(dt)),ncol=2)
  b <- matrix(rep(b,each=length(dt)),ncol=2)
  mean_v <- matrix(rep(mean_v,each=length(dt)),ncol=2)
  sd_v <- matrix(rep(sd_v,each=length(dt)),ncol=2)
  dt <- matrix(rep(dt,each=n_acc),nrow=n_acc)-t0
