...
transform.dmc <- function(par.df) 
{
  ...
  par.df[,c("A","b","t0","mean_v","sd_v","censor","gf")]
}


random.dmc <- function(n,p.df,model)
{
  rlba.norm.vGF(n,A=p.df$A,b=p.df$b,t0=p.df$t0,gf=p.df$gf, 
                mean_v=p.df$mean_v,sd_v=p.df$sd_v,censor=p.df$censor)
}
