...
{
  if (exists("transform_vec") && nrow(par.df) > 0) par.df <- duntrans(par.df)
...
random.dmc <- function(n, p.df, model, SSD = Inf, staircase = NULL, TRIALS = NULL) {
  rexgss(n,
    mu = p.df$mu, sigma = p.df$sigma, tau = p.df$tau, tf = p.df$tf[1], gf = p.df$gf[1],
...
# Returns vector of likelihoods for each RT in data (in same order)
{
  ...
  for (i in row.names(attr(data, "model"))) {
    if (!attr(data, "cell.empty")[i]) {
