...
# ARI version allowing for filled interval illusion, go(stop trial) = fi + fs*go(go trial)
#    External parameters types: tf, gf, mu, sigma, tau, muS, sigmaS, tauS, fi, fs 
#    Internal parameters types: tf, gf, mu, sigma, tau, fi, fs 
...
#   evidence accumulation model being used is present.
{
  ...
  par.df[,c("mu","sigma","tau","muS","sigmaS","tauS","tf","gf","fi","fs")]
}

random.dmc<- function(n,p.df,model, SSD=Inf,staircase=NULL,TRIALS=NULL)
{
  rexgssARI(n,mu=p.df$mu,sigma=p.df$sigma,tau=p.df$tau,fi=p.df$fi[1],fs=p.df$fs[1],
    tf=p.df$tf[1],gf=p.df$gf[1],SSD=SSD,staircase=staircase)
}
...
# Returns vector of likelihoods for each RT in data (in same order)
{
  ...
  for ( i in row.names(attr(data,"model")) ) if ( !attr(data,"cell.empty")[i] )
  {
    ...
    likelihood[ attr(data,"cell.index")[[i]] ] <-
      n1PDF.exgssARI(dt=data$RT[attr(data,"cell.index")[[i]]],
          mu=p.df$mu,
          sigma=p.df$sigma,
          tau=p.df$tau, 
          # Filled interval
