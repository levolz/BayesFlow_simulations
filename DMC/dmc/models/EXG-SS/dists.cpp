...
// [[Rcpp::export]]
NumericVector pexGAUS(NumericVector q,
...
// [[Rcpp::export]]
NumericVector dexGAUS(NumericVector x,
                   double mu = 5., double sigma = 1., double tau = 1.,
                   bool log_d = false) {
	int n = x.size();
	if (tau <= 0 || sigma <= 0) {
...
// [[Rcpp::export]]
NumericVector n1PDF_exg(NumericMatrix dt,
                       NumericVector mu, NumericVector sigma, NumericVector tau){
	int n = mu.size();
	NumericVector out(dt.nrow());
	out = dexGAUS(dt(0, _), mu[0], sigma[0], tau[0], false);
	for (int i = 1; i < n; i++){
		out = out * pexGAUS(dt(i, _), mu[i], sigma[i], tau[i], false, false);
	}
	return out;
}
...
NumericVector stopfn_exg(NumericVector t,
	...
	dt(0, _) = dt(0, _) - SSD;
	return n1PDF_exg(dt, mu, sigma, tau);
}
