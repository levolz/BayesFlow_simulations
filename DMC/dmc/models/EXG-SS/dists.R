...
# robust against small sigma cases.
{
  ...
  # if (sigma < 0.05*nu)
  if (sigma < 1e-4) {
    return(pexp(q - mu, 1 / nu, log = log.p, lower.tail = lower.tail))
  } # shfited exponential
...
# robust against small sigma cases.
{
  ...
  logfy <- ifelse(nu > 0.05 * sigma, -log(nu) - (z + (sigma^2 / (2 *
    nu))) / nu + log(pnorm(z / sigma)), dnorm(x,
    mean = mu, sd = sigma,
    log = TRUE
  ))
  if (log == FALSE) {
...
# tf = trigger failure probability, gf = go failure probability
{
  ...
  if (gf > 0) { # Setup for GO failure
    is.gf <- as.logical(rbinom(length(SSD), 1, gf))
  } else {
    ...
    if (any(is.na(staircase))) { # STOP fixed SSD
      ...
      for (i in c(1:n)) {
        ...
        if (runif(1) < tf) { # Trigger failure
          winner[i] <- which.min(dt[2:n_acc, i]) + 1
        } else {
...
{
  out <- rexgss(n,
    mu = mu, sigma = sigma, tau = tau, tf = tf, gf = 0,
...
# stop trials due to either sucessful stop or censoring.
{
  ...
  if (any(!is.rt)) {
    ...
    pC <- my.integrate(
      f = n1PDF.exg.vec, lower = UpperCensor, Si = 1, # dummy
    ...
    if (length(mu) > 2) {
      ...
      for (j in 3:length(mu)) {
        r[c(1, j - 1)] <- r[c(j - 1, 1)]
        pC <- pC + my.integrate(
          f = n1PDF.exg.vec, lower = UpperCensor,
