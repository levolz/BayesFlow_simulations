...
{
  if (!log) {
    dbeta((x - lower) / (upper - lower), shape1, shape2, log = FALSE) / (upper - lower)
  } else {
...
# Makes a list of prior distribution parameters.
{
  ...
  if (!all(dists %in% dist.types)) {
    stop(paste(
      "Unsupported distribution, allowable types are:",
  ...
  for (i in 1:length(p1)) {
    ...
    prior[[i]]$log <- TRUE
    if (!name.untrans) {
      attr(prior[[i]], "untrans") <- untrans[i]
    } else if (is.na(untrans[names(p1)[i]])) {
      attr(prior[[i]], "untrans") <- "identity"
    } else {
...
# log of prior density for p.vector
{
  ...
  for (i in names(p.vector)) {
    ...
    prior[i] <- do.call(
      paste("d", attr(p.prior[[i]], "dist"), sep = ""),
      p.prior[[i]]
    )
  }
  prior
...
# sample from prior
{
  ...
  for (i in 1:length(names(p.prior))) {
    p.prior[[i]]$n <- n
    prior[, i] <- do.call(
      paste("r", attr(p.prior[[i]], "dist"), sep = ""),
      p.prior[[i]][names(p.prior[[i]]) != "log"]
    )
    # prior[,i] <- do.call(attr(p.prior[[i]],"untrans"), list(prior[,i]))
  }
  prior
}
  ...
  if (is.null(samples)) # setup new samples
    {
      ...
      for (i in 1:n.chains)
      {
        ...
        while (any(is.infinite(summed_log_prior[1, i])) || any(is.nan(summed_log_prior[1, i])) ||
          any(is.infinite(log_likelihoods[1, i])) || any(is.nan(log_likelihoods[1, i]))) {
          ...
          if (!is.null(theta1) && (any(
            any(is.infinite(summed_log_prior[1, i])) || any(is.nan(summed_log_prior[1, i])) ||
              any(is.infinite(log_likelihoods[1, i])) || any(is.nan(log_likelihoods[1, i]))
          ))) {
            print(theta[i, , 1])
            stop("Bad theta1")
    ...
    if (!add & restart) { # start afresh
      ...
      if (is.logical(replace.bad.chains)) {
        ...
        replace.bad.chains <- unique(unlist(bad[unlist(lapply(bad, function(x) {
          length(x) > 0
        }))]))
        if (length(replace.bad.chains) == 0) {
...
# DEMCMC crossover update of one chain, data level
{
  ...
  # Old post
  if (force) {
    use.logprior[k] <- summed.log.prior(
      p.vector = use.theta[k, ], p.prior = p.prior,
      p.names = names(attr(attributes(data)$model, "p.vector"))
    )
  ...
  # new post
  log.prior <- summed.log.prior(
    p.vector = theta, p.prior = p.prior,
    p.names = names(attr(attributes(data)$model, "p.vector"))
  )
...
# DEMCMC migrate set, all chains, data level
{
  ...
  for (i in 1:n.groups) {
    ...
    propset.logprior[i] <- summed.log.prior(
      p.vector = thetaset[i, ],
      p.prior = p.prior, p.names = names(attr(attributes(data)$model, "p.vector"))
    )
    propset.loglike[i] <- sum(log.likelihood(
      p.vector = thetaset[i, ],
      data = data
    ))
...
get.dqp <- function(sim, facs, probs, n.post = NA, ns = NA, bw = "nrd0", reps = NA) {
  ...
  nok <- tapply(sim$RT, sim[, c(facs, "R")], function(x) {
    sum(!is.na(x))
  })
  ...
  if (!is.na(n.post) && (n.post > 1)) {
    ...
    ps <- tapply(
      sim$RT, cbind(sim[, c(facs, "R"), drop = FALSE], rep = repfac),
      function(x) {
        sum(!is.na(x))
      }
    )
  ...
  dens <- tapply(sim$RT, sim[, c(facs, "R"), drop = FALSE], function(x) {
    if (all(is.na(x))) {
      ...
      if (length(x[!is.na(x)]) < 2) {
        ...
        out <- try(density(x[!is.na(x)], bw = bw), silent = TRUE)
        if (class(out) == "try-error") NULL else out
      }
    }
  })
  for (i in 1:length(p)) {
    if (is.finite(p[i]) && !(p[i] == 0)) {
      if (!is.null(qs[i][[1]])) {
  ...
  dnd <- dimnames(dens)
  dnq <- dimnames(qs)
  dens <- apply(dens, 1:length(facs), function(x) {
    x
  })
  qs <- apply(qs, 1:length(facs), function(x) {
    x
  })
  if (is.null(dim(dens))) {
...
#   if (post.median) {
#     n.post <- 1
#     p.vector <- apply(thetas,2,median)
#     posts <- matrix(p.vector,nrow=1,dimnames=list(NULL,names(p.vector)))
...
#     if (post.median & expand>1) {
#       SSD <- rep(SSD,expand)
#       cvs <- data.frame(matrix(nrow=n.rep,ncol=0))
#     }
#   }
#   cat("\n")
...
#     if (!is.null(cvs))
#       sim[(1+(i-1)*n.rep):(i*n.rep),names(attr(tmp,"cvs"))] <- attr(tmp,"cvs")
#     if ( (i %% report) == 0) cat(".")
#   }
#   cat("\n")
...
#     if (post.median) {
#       sim <- sim[,-1]
#       attr(sim,"p.vector") <- posts[1,]
#     } else attr(sim,"data") <- samples$data
#     sim
#   } else {
...
# gglist=TRUE; probs.gglist=c(0.1, 0.5, 0.9); CI.gglist=c(0.025, 0.975)
#  censor=c(NA,NA); add.fac.fn=NULL; add.fac.name=NULL; new.R.fn=NULL
# samples=sStarns2[[3]]
...
post.predict.dmc <- function(samples, n.post = 100, probs = c(1:99) / 100, random = TRUE,
                             ...
                             save.simulation.as.attribute = FALSE, ignore.R2 = FALSE, censor = c(NA, NA),
                             gglist = TRUE, probs.gglist = c(0.1, 0.5, 0.9), CI.gglist = c(0.025, 0.975),
                             add.fac.fn = NULL, add.fac.name = NULL, new.R.fn = NULL, randomize = NA, block = NA)
                             # make list of posterior preditive density, quantiles and response p(robability)
# NB: quantiles only calcualted for 2 or more RTs
{
  model <- attributes(samples$data)$model
  ...
  cvs <- samples$data[, attr(model, "cvs"), drop = FALSE]
  attr(cvs, "row.facs") <- apply(apply(
    samples$data[, facs, drop = FALSE], 2, as.character
  ), 1, paste, collapse = ".")
  if (ignore.R2 & any(names(samples$data) == "R2")) {
    samples$data <- samples$data[, names(samples$data)[names(samples$data) != "R2"]]
  }
  ...
  colnames(thetas) <- dimnames(samples$theta)[[2]]
  if (is.na(n.post)) {
    use <- c(1:dim(thetas)[1])
  } else {
    if (random) {
      use <- sample(c(1:dim(thetas)[1]), n.post, replace = F)
    } else {
      use <- round(seq(1, dim(thetas)[1], length.out = n.post))
    }
  }
  ...
  if (!any(names(samples$data) == "SSD")) {
    ...
    if (is.null(facs)) {
      ...
      SSD <- unlist(tapply(samples$data$SSD, samples$data[, facs], identity))
      if (any(names(samples$data) == "TRIALS")) {
        TRIALS <- unlist(tapply(samples$data$TRIALS, samples$data[, facs], identity))
      } else {
        TRIALS <- NULL
      }
    }
  ...
  for (i in 1:n.post) {
    tmp <- simulate.dmc(posts[i, ], model,
      n = ns, SSD = SSD, cvs = cvs, TRIALS = TRIALS,
      randomize = randomize, block = block
    )
    if (ignore.R2) tmp <- tmp[, names(tmp)[names(tmp) != "R2"]]
    sim[(1 + (i - 1) * n.rep):(i * n.rep), names(tmp)] <- tmp
    if ((i %% report) == 0) cat(".")
  ...
  if (!is.na(censor[1])) {
    ...
    fast <- rep(FALSE, dim(sim)[1])
  }
  if (!is.na(censor[2])) {
    slow <- sim[, "RT"] > censor[2]
    cat(paste("Removing", sum(slow), "(", round(100 * mean(slow), 2)), " %) slow RTs\n")
  } else {
    slow <- rep(FALSE, dim(sim)[1])
  }
  ok <- !fast & !slow
  sim <- sim[ok, ]
  reps <- reps[ok]

  if (!is.null(add.fac.fn) & !is.null(add.fac.name)) {
    samples$data[[add.fac.name]] <- add.fac.fn(samples$data)
    sim[[add.fac.name]] <- add.fac.fn(sim)
    factors <- c(add.fac.name, factors)
  }
  if (!is.null(new.R.fn)) {
    samples$data$R <- new.R.fn(samples$data)
    sim$R <- new.R.fn(sim)
  }
  ...
  if (save.simulation) {
    ...
    if (gglist) {
      attr(out, "gglist") <-
        get.fitgglist.dmc(
          sim = cbind(reps, sim), data = samples$data, factors = factors, noR = FALSE,
          quantiles.to.get = probs.gglist, CI = CI.gglist
        )
    }
    out
...
run.unstuck.dmc <- function(samples, nmc = NA, report = 10, cores = 1,
                            ...
                            end.no.migrate = FALSE, farjump = NA)
...
# runs one final time with migration off.
{
  ...
  if (any(is.na(samples$theta[, , 2]))) {
    samples <- run.dmc(
      samples = samples, report = report, cores = cores,
      gamma.mult = gamma.mult, p.migrate = p.migrate, farjump = farjump
    )
  }
  ...
  repeat {
    ...
    samples <- run.dmc(samples.dmc(nmc = nmc, samples = samples),
      ...
      gamma.mult = gamma.mult, farjump = farjump
    )
  ...
  if (end.no.migrate) {
    samples <- run.dmc(samples.dmc(nmc = nmc, samples = samples),
      report = report, cores = cores, gamma.mult = gamma.mult, farjump = farjump
    )
  }
  samples
}
...
run.converge.dmc <- function(samples, nmc, report = 10, cores = 1, gamma.mult = 2.38,
                             cut = 1.1, max.try = 100, minN = NA, meanN = NA, farjump = NA,
...
# fulfilled will continue if necessary to get either min or mean effectiveSize
{
  ...
  if (any(is.na(samples$theta[, , 2]))) {
    samples <- run.dmc(
      samples = samples, farjump = farjump,
  ...
  if (!is.na(minN) & (gd <= cut)) {
    okN <- do.call(nfun, list(effectiveSize.dmc(samples))) > minN
  } else {
    okN <- TRUE
  }
...
# cut.flat.location=1/2;cut.flat.scale=1/2; max.try=20; n.add=NA; farjump=NA
# use.effectiveSize=TRUE
#
# report=1; cores=36; verbose=TRUE
...
RUN.dmc <- function(samples, cores = 1, report = 10, p.migrate = .05, max.try = 20,
                    ...
                    verbose = FALSE, gamma.mult = 2.38, farjump = NA, saveName = NULL) {
  ...
  FlatTests <- function(samples, p1 = 1 / 3, p2 = 1 / 3, cut.location = 0.25, cut.scale = Inf,
                        verbose = FALSE) {
    gmcmc <- function(samples) {
      mcmc(matrix(aperm(samples$theta, c(1, 3, 2)),
    ...
    # change in mean relative to IQR
  ...
  if (any(!is.finite(samples$log_likelihoods[samples$nmc, ]))) { # New samples
    ...
    samples <- run.dmc(samples,
      report = report, verbose = verbose,
      cores = cores, p.migrate = p.migrate, gamma.mult = gamma.mult, farjump = farjump
    )
  } else {
    do.migrate <- 0
  } # Samples already good, just want more.
  if (!is.null(saveName)) save(samples, file = saveName)

  if (!force && !is.null(attr(samples, "auto")) &&
    !is.na(attr(samples, "auto")) && attr(samples, "auto") != "GRID FAIL") {
    return(samples)
  } # If already sucessfully run then dont run again unless force=TRUE
  n.try <- 0
  repeat {
    # DO TESTS

    # Stuck check
    if (StuckTests(samples, cut = cut.unstuck, verbose = verbose)) { # Start again
      do.migrate <- 1
      get.new <- test.again <- TRUE
    } else {
      get.new <- test.again <- FALSE
    }

    # Stationarity check
    if (!get.new && any(FlatTests(samples,
      verbose = verbose,
    ...
    if ((!get.new & !test.again) &&
      MixTests(samples, verbose = verbose, cut = cut.converge, split = split)) {{ # Make longer
    ...
    if (get.new) { # Stuck chains, start again
      ...
      samples <- run.dmc(samples.dmc(samples = samples, nmc = 3 * n, thin = samples$thin),
        ...
        verbose = verbose, farjump = farjump
      )
    } else { # Test failed, update
      if (verbose) cat("Adding extra samples\n")
      samples <- run.dmc(samples,
        report = report, verbose = verbose,
        cores = cores, p.migrate = p.migrate * do.migrate,
        gamma.mult = gamma.mult, farjump = farjump
      )
    }
    if (!is.null(saveName)) save(samples, file = saveName)
...
pda.dmc <- function(rts, srt, n = length(srt), adjust = .8) {
  ...
  d <- density(srt,
    bw = bw, adjust = adjust,
    from = minrt - 3 * bw, to = maxrt + 3 * bw
  ) # Do smooth over region of data
  d$y[d$y < 0] <- 0
