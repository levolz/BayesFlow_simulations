...
theta.as.mcmc.list <- function(samples,start=1,end=NA,split=FALSE,thin=1,use.par=NA)
  ...
  # split doubles number of chains, first and second half
{
  ...
  if (any(is.na(use.par))) mcmc.list(lst) else {
    if (all(use.par %in% names(lst))) mcmc.list(lst[use.par]) else
      stop("use.par has names not in hyper")
  }
}



phi.as.mcmc.list <- function(hyper,start=1,end=NA,split=FALSE,thin=1,use.par=NA)
...
p.fun.dmc <- function(samples,fun,hyper=FALSE,ptype=1,pnams=NA) 
  # applies fun to samples
  {
   if (!hyper) {
     if (any(is.na(pnams))) 
       pnams <- dimnames(samples$theta)[[2]] 
...
thin.dmc <- function(hs,thin=10) {
    
   thin.one <- function(s,thin) {
     nmc_thin <- seq(thin,s$nmc,by=thin)
     s$theta <- s$theta[,,nmc_thin]
     s$summed_log_prior <- s$summed_log_prior[nmc_thin,]
     s$log_likelihoods <- s$log_likelihoods[nmc_thin,]
     s$nmc <- length(nmc_thin)
     s$thin <- s$thin*thin
     s$start=1
     s
   }
   
   if (thin >= hs[[1]]$nmc) stop("Thin value to large.")
   for (i in 1:length(hs))
     hs[[i]] <- thin.one(hs[[i]],thin)
   if ( any(names(attributes(hs))=="hyper") ) {
     hyper <- attr(hs,"hyper")  
     nmc_thin <- seq(thin,hyper$nmc,by=thin)
     hyper$phi[[1]] <- hyper$phi[[1]][,,nmc_thin]
     hyper$phi[[2]] <- hyper$phi[[2]][,,nmc_thin]
     hyper$h_summed_log_prior <- hyper$h_summed_log_prior[nmc_thin,]
     hyper$h_log_likelihoods <- hyper$h_log_likelihoods[nmc_thin,]
     hyper$nmc <- length(nmc_thin)
     hyper$thin <- hyper$thin*thin
     hyper$start=1
     attr(hs,"hyper") <- hyper
   }
   hs
  ...
  # split doubles the number of chains by spliting into 1st and 2nd halves.
{
  ...
  gelman.diag.robust <- function(mcl,autoburnin,transform) 
  {
    gd <- try(gelman.diag(mcl,
          autoburnin=autoburnin,transform=transform),silent=TRUE)
  ...
  if ( hyper ) {
    ...
    gelman.diag.robust(phi.as.mcmc.list(hyper,start=start,end=end,split=split),
                autoburnin=autoburnin,transform=transform)
  } else {
    if ( !is.null(samples$theta) ) {
      if (is.na(end)) end <- samples$nmc
      gelman.diag.robust(theta.as.mcmc.list(samples,start=start,end=end,split=split),
                  autoburnin=autoburnin,transform=transform) 
    } else {
      ...
      for (i in 1:length(samples)) {
        ...
        out[[i]] <- gelman.diag.robust(theta.as.mcmc.list(samples[[i]],start=start[i],
  ...
  # Calclaute WAIC  
{
  ...
  outl <- list(p_waic=out$estimates["p_waic",1],se_p=out$estimates["p_waic",2],
               waic=out$estimates["waic",1],se_waic=out$estimates["waic",2])
  if (mc_se) {
    n.chains <- dim(trial_log_likes)[2]
    waics <- vector(mode="list",length=n.chains)
    for (i in 1:n.chains) waics[[i]] <- waic(trial_log_likes[,i,])
    mc <- sd(unlist(lapply(waics,function(x){x$estimates["waic",1]})))/sqrt(n.chains)
    print(c(p=outl$p_waic,se_p=outl$se_p,waic=outl$waic,se_waic=outl$se_waic,
      mc_se_waic=mc),...) 
  } else 
    print(c(p=outl$p_waic,se_p=outl$se_p,waic=outl$waic,se_waic=outl$se_waic),...)
  if (save) out
  ...
  # Calcuate looic 
{
  ...
  outl <- list(p_loo=out$estimates["p_loo",1],se_p=out$estimates["p_loo",2],
               looic=out$estimates["looic",1],se_looic=out$estimates["looic",2])
  ...
  if (mc_se) {
    ...
    mc <- sd(unlist(lapply(loos,function(x){x$estimates["looic",1]})))/sqrt(n.chains)
    print(c(p=outl$p_loo,se_p=outl$se_p,looic=out$llooic,se_looic=outl$se_looic,
      mc_se_loo=mc),...) 
  } else 
    print(c(p=outl$p_loo,se_p=outl$se_p,looic=outl$looic,se_looic=outl$se_looic),...)
  if (save) out
}
...
add.par.dmc <- function(hsamples,fun,p.name,n.iter=NA,n.chain=NA) 
  # Adds computed variable to samples  
{
  add.par <- function(x,fun,p.name,n.chain,n.iter) {
    d <- dim(x$theta)
    p <- apply(x$theta,c(1,3),fun)
    d[2] <- d[2]+1
    dn <- dimnames(x$theta)
    dn[[2]] <- c(dn[[2]],p.name)
    x$theta <- aperm(array(c(aperm(x$theta,c(1,3,2)),p),dim=d[c(1,3,2)]),
      c(1,3,2))[1:n.chain,,1:n.iter]
    dimnames(x$theta) <- dn
    x
  }
  
  if ( any(names(hsamples)=="theta") ) 
    add.par(hsamples,fun,p.name) else {
      if ( is.na(n.chain) )
        n.chain <- min(unlist(lapply(hsamples,function(x){dim(x$theta)[1]})))
      if ( is.na(n.iter) )
        n.iter <- min(unlist(lapply(hsamples,function(x){dim(x$theta)[3]})))
      lapply(hsamples,add.par,fun=fun,p.name=p.name,
        n.chain=n.chain,n.iter=n.iter)  
    }
}


cor.plausible <- function(hsamples,cv=NULL,p.name=NULL,plot=FALSE,cor.plot=FALSE,
                   pick=1000,xlab="r",ylab="Density",main=p.name,fun=NULL,...)
  # Correlations for each interation and chain with of p.name with cv$p.name
  # Or if p.name length 2 plausibe correlation between two parameters
{
  if ( (length(p.name)==1) | (length(fun)==1) ) {
  ...
  } else if ( (length(p.name)==2) | (length(fun)==2) ) {
    if ( is.null(fun) ) {
      subject.pars1 <- do.call(rbind,lapply(hsamples,function(x){
        as.vector(x$theta[,p.name[1],])}))  
      subject.pars2 <- do.call(rbind,lapply(hsamples,function(x){
        as.vector(x$theta[,p.name[2],])}))  
    } else {
      if (!is.list(fun)) {
        fun1 <- fun; fun2 <- fun
      } else {
        fun1 <- fun[[1]]; fun2 <- fun[[2]]
      }
      subject.pars1 <- do.call(rbind,lapply(hsamples,function(x){
          pars <- aperm(x$theta,c(1,3,2));
          pars <- matrix(as.vector(pars),ncol=dim(pars)[3]);
    ...
    subject.pars <- array(c(subject.pars1,subject.pars2),
      dim=c(dim(subject.pars1),2))
    out <- apply(subject.pars,2,function(x){cor(x[,1],x[,2])})
  } else stop("p.name or fun must be length 1 or 2")
  if ( plot ) {
    if (cor.plot) {
...
# 
# hsamples <- yo[[1]]; fun=within.fun;pnames=within.pnames; hyper=FALSE; hpar=1

compare.p=function(hsamples,fun=NULL,pnames=NULL,pretty=NULL,hyper=FALSE,hpar=1,
  subsample=NA,show.plot=FALSE,main="",xlab="",show.legend=TRUE,lpos="topleft",
  xlim=NULL,show.table=TRUE,lo.p=.025,hi.p=.975,digits=3,line0=TRUE,...)
{
  ...
  if (hyper) {
    ...
    if ( is.null(n) ) {
      ...
      return(apply(do.call(rbind,cmats),2,mean))
    } 
...
# between.fun=NULL; within.pnames=NULL; within.fun=NULL
# hyper=TRUE;hpar=1;pretty=NULL;show.plot=TRUE;main="";xlab=""
# show.legend=TRUE;lpos="topleft";xlim=NULL
# show.table=TRUE;lo.p=.025;hi.p=.975;digits=3;line0=TRUE
# 
# fits=yo;within.pnames=c("t0.block")

compare.ps <- function(fits,between.fun=NULL,within.pnames=NULL,within.fun=NULL,
  hyper=TRUE,hpar=1,pretty=NULL,show.plot=TRUE,main="",xlab="",show.legend=TRUE,
  lpos="topleft",xlim=NULL,show.table=TRUE,lo.p=.025,hi.p=.975,digits=3,line0=TRUE,
  silent=TRUE,...)
  # Tests differences in parameters among two or more fits  
{
  ...
  avs <- lapply(fits,compare.p,fun=within.fun,pnames=within.pnames,
         hyper=hyper,hpar=hpar)
  ns <- unlist(lapply(avs,length))
  if (!all(ns[1]==ns[-1]) & !silent)
      warning(paste("Different number of samples per fit, using min =",min(ns)))
  ns <- min(ns)
  avs <- lapply(avs,function(x){x[1:ns]})
...
subject.average.ci <- function(hsamples,fun=NULL,pnames=NULL,lo.p=.025,hi.p=.975,pretty=NULL,...)
{
  ...
  n <- min(unlist(lapply(hsamples,function(x){dim(x$theta)[3]})))
  cmats <- lapply(hsamples,function(x){
    apply(matrix(as.vector(aperm(x$theta[,,1:n],c(1,3,2))),ncol=dim(x$theta)[2],
      dimnames=list(NULL,dimnames(x$theta)[[2]])),1,fun,...)
  })
  n <- dim(cmats[[1]])[1]
  if (!is.matrix(cmats[[1]])) {
    nsamp <- min(unlist(lapply(cmats,function(x){length(x)}))) 
    cmats <- lapply(cmats,function(x){x[1:nsamp]})
    av <- apply(
      array(do.call(rbind,cmats),dim=c(n,length(hsamples))),1,mean)
    quantile(av,probs=c(lo.p,.5,hi.p))
  } else {
    nsamp <- min(unlist(lapply(cmats,function(x){dim(x)[2]})))
    cmats <- lapply(cmats,function(x){x[,1:nsamp]})
    av <- apply(
