  ...
  # case it provides a summary of p(NA)
{
  ...
  if ( !any(is.na(C)) ) {
    ...
    legend(rpos,paste(nams,ps),bty="n")
    legend(lpos,xjust=0, inset=c(0,0.1), c("correct","error"),bty="n", 
    ...
    legend(rpos,paste(nams,ps),lty=ltys,bty="n",lwd=lwd)    
    if ( save.density ) dns
  }
}

# p.name="A";min.p=.5; max.p=1.5;n.point=100;digits=2;ylim=NA; verbose=TRUE
profile.dmc <- function(p.name,min.p,max.p,p.vector,data,draw.true=TRUE,
                        n.point=100,digits=2,ylim=NA,verbose=FALSE) 
  ...
  # returns the maximum (on a grid of resolution n.point) 
{
  ...
  if (draw.true) abline(v=p.vector[p.name],lty=3)
  ll[ll==max(ll)]
}
...
plot.dmc <- function(samples,hyper=FALSE,location=FALSE,scale=FALSE,
                     start=1,end=NA,thin=1,save.ll=FALSE,use.par=NA,
                     main.pll=NULL,pll.chain=FALSE,pll.together=TRUE,pll.barplot=FALSE,
                     ...
                     ...)
{
  ...
  if ( hyper ) {
    ...
    if ( pll.chain | pll.barplot ) {
      ...
      if (!auto.layout) par(mfrow=layout)
      plot(window(phi.as.mcmc.list(hyper,start=start,end=end,thin=1,use.par=use.par)),
    ...
    if ( pll.chain | pll.barplot ) {
      ...
      if (!auto.layout) par(mfrow=layout)
      plot(window(theta.as.mcmc.list(samples,thin=thin,use.par=use.par),start=start,end=end),
           auto.layout=auto.layout,density=density,smooth=smooth,
           p.prior=p.prior,
           prior.col = prior.col, post.col = post.col,
           prior.lwd = prior.lwd, post.lwd = post.lwd,
    ...
    for ( i in 1:ni ) if ( !all(unlist(lapply(pp[[1]][[i]],is.null))) ) {
      ...
      if ( !is.na(pos) ) {
        if ( model.legend ) legend(pos,c(paste(dname,lgnd)[is.in],paste(mname,lgnd)[is.in]),
          lty=c(ltys[is.in],ltys[is.in]),
          lwd=c(lwds[is.in]*data.lwd.mult,lwds[is.in]),bty="n",
          col=c(rep(data.col, length(is.in)),rep("black",length(is.in))) ) else
        legend(pos,paste(dname,lgnd)[is.in],lty=ltys[is.in],
               lwd=lwds[is.in]*data.lwd.mult,bty="n",
               col=rep(data.col, length(is.in)) )
      }
    }
  }
...
plot_SS_srrt.dmc <- function(data,sim=NULL,minN=1,ci=.99,sim.line=TRUE,
  main="Median signal-respond RTs",xlab=NA,sim.pch=1,len=.05,
  do.correct=FALSE,Cfun=function(x){as.numeric(x$S)==(as.numeric(x$R)-1)},
  plot.it=TRUE,violin=c(NA,.05)[1],ylab="Median SRRT (s)",ylim=NA,only.ssrt=TRUE,
  probs=NA,n.intervals=5,low.interval=NA,percentile.av=FALSE,cut.on.data=TRUE,
  xlabels=NULL,unique.breaks=TRUE,point.col="black",add=FALSE,data.lty=1,data.col="black")
  ...
  
{
  ...
  if ( plot.it & !fit ) { # Signal respond plot
    ...
    if (add) points(xvals[ok],mrt[ok],pch=16) else {
    ...
    lines(xvals[ok],mrt[ok],lwd=2,lty=data.lty,col=data.col)
  }
  ...
  if ( fit ) {
    ...
    mrt.med <- apply(mrt.sim,2,quantile,probs=.5,na.rm=TRUE)
    ...
    if ( plot.it ) { # Signal respond plot
      ...
      if (is.na(violin)) {
        points(xvals[ok],mrt.med[ok],pch=sim.pch)
        arrows(xvals[ok],mrt.med[ok],xvals[ok],mrt.lo[ok],length=len,angle=90)
        arrows(xvals[ok],mrt.med[ok],xvals[ok],mrt.hi[ok],length=len,angle=90)
      } else if ( as.logical(abs(violin)) ) for (i in SSDs[ok]) {
        vioplot(mrt.sim[!is.na(mrt.sim[,i]),i],at=xvals[i],add=TRUE,
                col="lightgrey",wex=abs(violin))
      }
      if (sim.line) lines(xvals[ok],mrt.med[ok],lty=3)
    }
...
# sim=NULL;minN=1;plot.it=TRUE;violin=.05; add=FALSE; xlim=NULL; data.pch=16
...
# xlabels=NULL;unique.breaks=TRUE;point.col="black"; subSet <- NA; absolute.x=TRUE
# 
# data=hYoungStop;sim=ppY;n.intervals=6
# violin=NA;percentile.av=FALSE;sim.line=sim.line;subSet=c(BT="block")
# xlim=c(.05,.8);ylim=c(.3,.7);main="Block";xlab="SSD (seconds)";zscore=zscore



plot_SS_if.dmc <- function(data,sim=NULL,minN=1,plot.it=TRUE,violin=.05,ci=95,len=.05,
  ylab="p(Respond)",xlab=NA,ylim=NA,sim.line=FALSE,main="Inhibition function",
  probs=NA,n.intervals=5,low.interval=NA,percentile.av=TRUE,absolute.x=TRUE,cut.on.data=TRUE,
  xlabels=NULL,unique.breaks=TRUE,point.col="black",subSet=NA,
  data.pch=16,sim.pch=1,sim.lty=3,data.lty=1,data.col="black",add=FALSE,xlim=NULL)
  # Plots inhibiiton function, optionally with violin plots and invisibly 
  ...
  # NB1: p for n=1 is meaningless, so p=NA in such cases
  # NB2: Does not plot violins if violin=NA or 0 or FALSE, otherwise absolute value  
  #      of violin gives "wex", width of density plot. If NA then plots median and 
  #      ci width credible interval with bar ends of length len.
  # NB3: sim.line joins violin medians with dotted line.
  # NB4: subSet allows picking out cells in data and simulation
  # NB5: If percentile.av=FALSE then default absolute.x=TRUE plots interval mid-
  #      points, otherwise labels axis with ranges.
  # NB6: Other options similar to SSRT plot
{
  fit <- !is.null(sim)
  if (is.na(xlab)) xlab <- ifelse(is.data.frame(data),"SSD (s)","SSD")
  if ( !is.data.frame(data) ) { # average over subjects
    ...
    if (!any(is.na(subSet))) {
      ok <- lapply(data,function(x){x[[names(subSet[1])]]==subSet[1]}) 
      if (length(subSet)>1) for (i in 2:length(subSet)) for (j in 1:length(data))
        ok[[j]] <- ok[[j]] & data[[j]][[names(subSet[i])]]==subSet[i]
      for (j in 1:length(data)) data[[j]] <- data[[j]][ok[[j]],]
      if (fit) {
        ok <- lapply(sim,function(x){x[[names(subSet[1])]]==subSet[1]}) 
        if (length(subSet)>1) for (i in 2:length(subSet)) for (j in 1:length(sim))
          ok[[j]] <- ok[[j]] & sim[[j]][[names(subSet[i])]]==subSet[i]
        for (j in 1:length(sim)) sim[[j]] <- sim[[j]][ok[[j]],]
      }  
    }
    
    # HAS PROBLEMS WITH LOWER BOUND
    # if (zscore) for (j in 1:length(data)) {
    #   rt <- data[[j]]$RT[data[[j]]$SS=="GO"]; rt <- rt[!is.na(rt)]  
    #   data[[j]]$SSD <- (data[[j]]$SSD-mean(rt))/sd(rt)
    # }
    # if (fit & zscore) for (j in 1:length(sim)) {
    #   rt <- sim[[j]]$RT[sim[[j]]$SS=="GO"]; rt <- rt[!is.na(rt)]  
    #   sim[[j]]$SSD <- (sim[[j]]$SSD-mean(rt))/sd(rt)
    # }
    ...
    av.plot <- FALSE
    if (!any(is.na(subSet))) {
      ok <- data[[names(subSet[1])]]==subSet[1] 
      if (length(subSet)>1) for (i in 2:length(subSet)) 
        ok <- ok & data[[names(subSet[i])]]==subSet[i]
      data <- data[ok[[j]],]
      if (fit) {
        ok <- sim[[names(subSet[1])]]==subSet[1] 
        if (length(subSet)>1) for (i in 2:length(subSet)) 
          ok <- ok & sim[[names(subSet[i])]]==subSet[i]
        sim <- sim[ok[[j]],]
      }  
    }
    if (fit) sim <- sim[is.finite(sim$SSD),]
  ...
  if (!percentile.av & absolute.x) {
      los <- unlist(lapply(lapply(lapply(strsplit(names(xvals),split=","),function(x){x[1]}),
        strsplit,split="(",fixed=TRUE),function(x){as.numeric(x[[1]][2])}))
      his <- unlist(lapply(lapply(lapply(strsplit(names(xvals),split=","),function(x){x[2]}),
        strsplit,split="]",fixed=TRUE),function(x){as.numeric(x[[1]][1])}))
      xvals <- los+(his-los)/2
      plot.axis <- TRUE
  } else plot.axis <- FALSE

  if ( !fit & plot.it ) { 
    if (any(is.na(ylim))) ylim <- c(0,1)
    if (plot.axis | add) {
      if (add) points(xvals,pR,pch=data.pch) else
        plot(xvals,pR,ylim=ylim,pch=data.pch,xlab=xlab,ylab=ylab,main=main)  
    } else {
      plot(xvals,pR,ylim=ylim,pch=data.pch,xlab=xlab,ylab=ylab,main=main,xaxt="n")
      axis(1,xvals,xlabs)
    }
    points(xvals,pR,pch=data.pch)
    lines(xvals,pR,lwd=2,lty=data.lty,col=data.col)
  }
  ...
  if ( fit ) {
    ...
    if ( plot.it ) { # Inhibition Function
      ...
      if (plot.axis | add) {
        if (add)
          points(xvals,pR,pch=data.pch) else
          plot(xvals,pR,ylim=ylim,pch=data.pch,xlab=xlab,ylab=ylab,main=main,xlim=xlim) 
        } else {
          plot(xvals,pR,ylim=ylim,pch=data.pch,xlab=xlab,ylab=ylab,main=main,
            xaxt="n",xlim=xlim)
          axis(1,xvals,xlabs)
      ...
      if (is.na(violin)) {
        p <- apply(pR.sim,2,quantile,probs=c((1-ci/100)/2,.5,1-(1-ci/100)/2))
        points(xvals,p[2,],pch=sim.pch)
        arrows(xvals,p[2,],xvals,p[1,],length=len,angle=90)
        arrows(xvals,p[2,],xvals,p[3,],length=len,angle=90)
      } else if ( as.logical(abs(violin)) ) for (i in SSDs) {
        vioplot(pR.sim[,i],at=xvals[i],add=TRUE,col="grey",wex=abs(violin))
      }
      if (!sim.line) lines(xvals,pR,lwd=2,lty=data.lty,col=data.col) else
        lines(xvals,apply(pR.sim,2,median,na.rm=TRUE),lty=sim.lty)
