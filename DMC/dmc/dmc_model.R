...
p.list.dmc <- function(p.vector,model,cells,n1order=TRUE,cvs=NULL,
                       n1.index=NULL,do.trans=FALSE)
  ...
  # "n1" order (used by likelihood.dmc)  
{
  ...
  # Make parameter array: parameters x data rows x accumulators
  parr <- array(
...
#     1-par.mat[,dimnames(par.mat)[[2]]=="z"]
  ...
  p.list <- transform.dmc(p.list,do.trans=do.trans)
  
  if ( n1order ) 
    p.list <- lapply(p.list,function(x){matrix(x[n1.index],ncol=dim(x)[2])})
  if ( !is.null(cvs) ) attr(p.list,"cvs") <- cvs
  p.list
}

# SSD=Inf;staircase=NA;TRIALS=NA; cvs=NULL; adapt=FALSE; block=NA; randomize=NA; save.adapt=FALSE
# adapt=TRUE; randomize=c("LS","RS"); cvs <- cvs1; save.adapt=TRUE
# 
# randomize="S"

simulate.dmc <- function(p.vector,model,n=2,randomize=NA,block=NA,
  cvs=NULL,SSD=Inf,staircase=NA,TRIALS=NA,adapt=FALSE,save.adapt=FALSE)
  ...
  # randomize and block allow control of trial order
...
{  
   ...
   repeat.SSD.random.dmc <- function(ni,p.df,model,SSD,staircase,TRIALS,maxTry=100) {
    out <- random.dmc(ni,p.df,model,
      SSD=SSD,staircase=staircase,TRIALS=TRIALS) 
    ndone <- dim(out)[1]
    ntry <- 0
    if (ndone < ni) repeat {
      out <- rbind(out,random.dmc(ni- ndone,p.df,model,
        SSD=SSD,staircase=staircase,TRIALS=TRIALS))
      ndone <- dim(out)[1]
      if (ndone==ni) break
      ntry <- ntry+1
      if (ntry>maxTry) stop(paste("Failure to get enough samples after",ntry,"tries!"))
    }
    out
  } 

    
  repeat.random.dmc <- function(ni,p.df,model,maxTry=100) {
    out <- random.dmc(ni,p.df,model) 
    ndone <- dim(out)[1]
    ntry <- 0
    if (ndone < ni) repeat {
  ...
  names(facs) <- fnams
  if (!any(is.na(randomize)) && !all(randomize %in% fnams))
    stop("randomize argument must contain factor names")
  if (!any(is.na(block)) && !all(block %in% fnams))
    stop("block argument must contain factor names")
  if ((!any(is.na(randomize)) & !any(is.na(block))) && 
      !all(sort(c(randomize,block))==sort(fnams)))
    stop("Together randomize and block arguments must include all factors")
  ...
  if ( attr(model,"type") %in% c("normDK","norm2C","norm3C","MTR") ) # Multi-threshold race model
  {
    data$R2 <- NA
    R2.name <- "R2"
  } else R2.name <- NULL
  ...
  if ( is.SSD )   # Stop signal model 
  {
    for ( i in 1:dim(facs)[1] ) if ( n[i]>0 ) 
    {
      ...
      data[row1:rown,c("RT","R",R2.name,SSD.name,TRIALS.name)] <- 
        suppressWarnings(repeat.SSD.random.dmc(n=n[i],p.df,model,
          SSD=SSD[row1:rown],staircase=staircase,TRIALS=TRIALS[row1:rown]))
      #Call the random function in the model file.
      #TRIALS defaults to null and is not used for exgss
      row1 <- rown+1 
    
    }
  } else if (!adapt)  {  # If not SSD model only n[i],p.df and model need be called
    ...
    for ( i in 1:dim(facs)[1] ) if ( n[i]>0 ) 
    {
      ...
      p.df <- p.df.dmc(p.vector,i,model,n1order=FALSE,
          cvs=cvs[attr(cvs,"row.facs")==cv.lookup[i],,drop=FALSE])
      data[row1:rown,c("RT","R",R2.name,SSD.name,TRIALS.name)] <- 
          suppressWarnings(repeat.random.dmc(n[i],p.df,model)) 
  ...
  # Reorder data
  if ( !any(is.na(randomize)) )  {
    if ( any(is.na(block)) ) block <- fnams[!(fnams %in% randomize)]
    rand <- fnams[(fnams %in% randomize)]
    if ( length(block)==0 ) reorder <- order(runif(dim(data)[1])) else {
      reorder <- 1:dim(data)[1]
      if ( length(rand)==1 ) first <- levs[[rand]][1] else
        first <- paste(unlist(lapply(levs[rand],function(x){x[1]})),collapse="") 
      ri <- apply(facs[rand],1,paste,collapse="")
      bi <- apply(facs[ri==first,block,drop=FALSE],1,paste,collapse="")
      bdat <- apply(data[block],1,paste,collapse="")
      for (i in bi) reorder[bdat==i] <- 
        reorder[bdat==i][order(runif(dim(out[bdat==i,])[1]))]
      reorder <- reorder[order(bdat)]
    }
    data <- data[reorder,]
    cv.out <- cv.out[reorder,,drop=FALSE]
  }
  
  if ( adapt ) { # Fill in data
    p.list <- p.list.dmc(p.vector,model=model,n1order=FALSE,cvs=cv.out,
        cells=paste(apply(data[,names(attr(model,"factors")),drop=FALSE],1,
          paste,collapse="."),attributes(model)$responses[1],sep="."),do.trans=TRUE)
    # Factors 
    attr(p.list,"facs") <- data[,fnams,drop=FALSE]
    attr(p.list,"cvs") <- cv.out
    adapt.out <- random.dmc(p.list,model,save.adapt)
    data[,c("RT","R")] <- adapt.out[,c("RT","R")]
    cv.out <- attr(adapt.out,"cvs")
  }
  ...
  attr(data,"cvs") <- cv.out
  if ( exists("adapt.out") && !is.null(attr(adapt.out,"adapt")) ) 
      attr(data,"adapt") <- attr(adapt.out,"adapt")
  data
}
...
data.model.cvs.dmc <- function(p.vector,model,data,factor.names,cv.names,n.pda=1e4) 
  ...
  dat <- simulate.dmc(p.vector,model=model,n=ns,cvs=cvs)
  dat <- cbind(attributes(dat)$cvs,dat)
  data.model.dmc(dat,model,n.pda = n.pda)
}
