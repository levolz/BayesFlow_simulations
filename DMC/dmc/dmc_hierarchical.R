...
h.simulate.dmc <- function(model,ps=NA,ns=1,n=2,randomize=NA,block=NA,
                           SSD=Inf,p.prior=NA,staircase=NA,cvs=NULL,
                           subject.cv=NULL,save.adapt=FALSE) # ,by.cell=TRUE
...
# cvs: list of covariate data frames, one per subject
...

{
  ...
  dati <- simulate.dmc(ps[1,],modeli,n[1,],randomize=randomize,block=block,cvs=cvs[[1]],
      SSD=SSD[datr],staircase = staircase,save.adapt=save.adapt) # ,by.cell=by.cell
  if (save.adapt) {
    adapt <- vector(mode="list",length=ns)
    adapt[[1]] <- attr(dati,"adapt")
  }
  data <- cbind(s=rep(1,length(datr)),dati)  
  if (ns>1) for (i in 2:ns)
  {
    if (subject.models) modeli <- model[[i]] else modeli <- model
    datr <- (ndatai[i]+1):(ndatai[i+1])
    dati <- simulate.dmc(ps[i,],modeli,n[i,],randomize=randomize,block=block,cvs=cvs[[i]],
        SSD=SSD[datr],staircase = staircase,save.adapt=save.adapt) # ,by.cell=by.cell
    data <- rbind(data,cbind(s=rep(i,length(datr)),dati))  
    if (save.adapt) adapt[[i]] <- attr(dati,"adapt")
  }  
  data$s <- factor(data$s)
  attr(data,"parameters") <- ps
  if (save.adapt) attr(data,"adapt") <- adapt
...
#     if (any(is.na(factors))) factors <- 
#         names(attributes(attributes(samples[[1]]$data)$model)$factors)
...
# add.fac.fn=NULL; add.fac.name=NULL; new.R.fn=NULL

# samples=hLBAB; gglist=TRUE

h.post.predict.dmc <- function(samples,n.post=100,probs=c(1:99)/100,bw="nrd0",
  save.simulation=FALSE,factors=NA,save.subject.posts=FALSE,cores=1,ignore.R2=FALSE, 
  gglist= FALSE, probs.gglist =c(0.1, 0.5, 0.9), CI.gglist =c(0.025, 0.975),
  censor=c(NA,NA),add.fac.fn=NULL,add.fac.name=NULL,new.R.fn=NULL,randomize=NA,block=NA)
  # apply post.predict to each subject
{
  os <- get.os()
  if ( cores>1 & length(samples)>1 & os=="windows") { 
    ...
    out <- sfLapply(samples,post.predict.dmc,n.post=n.post,probs=probs,bw=bw,
                    factors=factors,save.simulation=save.simulation,gglist=TRUE,
                    save.simulation.as.attribute=TRUE,ignore.R2=ignore.R2,censor=censor,
                    add.fac.fn=add.fac.fn,add.fac.name=add.fac.name,new.R.fn=new.R.fn,
  ...
  if ( !save.simulation ) { # Get averages
    ...
    if (!is.null(factors)) {
      ...
      if (!all(factors %in% c(add.fac.name,facs))) 
        stop(paste("Factors argument must contain one or more of:",
          paste(c(add.fac.name,facs),collapse=",")))
    }
    
    if (!is.null(add.fac.fn) & !is.null(add.fac.name)) {
      dat[[add.fac.name]] <- add.fac.fn(dat)
      factors <- c(add.fac.name,factors)
    }
    if (!is.null(new.R.fn)) dat$R <- new.R.fn(dat)
...
# cores=1;report=10;p.migrate=.05;h.p.migrate=.05
# max.try=100;cut.unstuck=10;cut.flat.location=.5;cut.flat.scale=.5
# cut.converge=1.1;split=TRUE;minN=NA;meanN=NA;use.effectiveSize=TRUE
# n.add=NA;force=FALSE;farjump=NA;thorough=TRUE;verbose=FALSE
# gamma.mult=2.38;h.gamma.mult=NA;subjects.to.cores=TRUE;slaveOutfile=NULL
  ...
  if ( !is.null(hsamples$theta) ) {
    ...
    if ( cores == 1 | !subjects.to.cores ) 
      hsamples <- lapply(hsamples,RUN.dmc,cores=cores,report=report,
                         ...
                         cut.converge=cut.converge,split=split,force=force,
...
# 
# model.dir="EXG-SS";model.file="exgSScens.R";user="ajh296";n.add=40;minN=2000
...
)
{
  ...
  "\n",module_load,"\n","cd $PBS_O_WORKDIR\nRscript --no-restore --no-save run-",fname,".R\nexit 0",sep=""),
...
# Fix for occasonal mysterious fails to collect
collect.bad.grid <- function(fname,jnum,froot="s",verbose=TRUE)
{
  oname <- load(paste(fname,"RData",sep="."))
  hsamples <- get(oname)
  cat("Harvesting files: ")
  ...
  file.remove(paste("run-",fname,".sh",sep=""))
  unlink(paste(fname,".o",jnum,".*",sep=""))
  tmp <- grep(".o",dir(pattern=paste0(fname)),value=TRUE,fixed=TRUE)
  for (i in tmp) file.remove(i)
}
