...
######### Truncated normal from the msm package
...
rtnorm <- function (n, mean = 0, sd = 1, lower = -Inf, upper = Inf) 
{
    ...
    alg <- ifelse(((lower > upper) | nas), -1, ifelse(((lower < 
        0 & upper == Inf) | (lower == -Inf & upper > 0) | (is.finite(lower) & 
...
get.dqp <- function(sim,facs,probs,n.post=NA,ns=NA,bw="nrd0",reps=NA) {
    ...
    if ( !is.na(n.post) && (n.post>1) ) { 
      if (!any(is.na(reps))) repfac <- reps else
...
#  censor=c(NA,NA); post.median=FALSE; expand=1
# 
# samples=hsamplesA[[1]]; n.post=2; acc.code="S"
...
{ 
  ...
  if ( save.simulation ) {
    ...
    if (!is.na(acc.code)) {
      if (!any(acc.code %in% factors)) stop("acc.code must be a factor")
      ok <- sim$R != "NR"
      if ( all(ok) ) {
        sim$R <- factor(as.numeric(sim$R[ok])==as.numeric(sim[[acc.code]][ok]))
      } else {
        crct <- as.numeric(sim$R[ok])-1==as.numeric(sim[[acc.code]][ok])
        sim$R <- as.character(sim$R)
        sim$R[ok] <- crct
        sim$R <- factor(sim$R,levels=c("NR","FALSE","TRUE")) 
      }
      sim <- sim[,names(sim)!=acc.code]
      ok <- samples$data$R != "NR"
      if ( all(ok) ) {
        samples$data$R <- factor(as.numeric(samples$data$R[ok])==as.numeric(samples$data[[acc.code]][ok]))
      } else {
        crct <- as.numeric(samples$data$R[ok])-1==as.numeric(samples$data[[acc.code]][ok])
        samples$data$R <- as.character(samples$data$R)
        samples$data$R[ok] <- crct
        samples$data$R <- factor(samples$data$R,levels=c("NR","FALSE","TRUE")) 
      }
      samples$data <- samples$data[,names(samples$data)!=acc.code]
      factors <- factors[factors!=acc.code]
    }
  ...
  # fulfilled will continue if necessary to get either min or mean effectiveSize
{
  ...
  if ( any(is.na(samples$theta[,,2])) ) {
    samples <- run.dmc(samples=samples,farjump=farjump, 
